...
```java
//
const bmr = 651.0 + 13.7 * bodyMass + 5.0 * height - 6.8 * age;
const bmi = bodyMass / ((height / 100) ** 2);
const workload = (workload3min + workload6min) / 3;
const volRel = workload / bodyMass;
const vlamax = calculateVLAmax(vo2max, bmi, bodyMass, workload, sprintPower);

function calculateVLAmax(vo2max, bmi, weight, workload, sprintPower) {
  var maderFormula = 0.02049 / (workload / weight) * vo2max * (bmi / 22) * (1 + 0.000025 * age - 0.0000001 * weight);
  var sprintContribution = 0.000004 / (workload / weight) * sprintPower * (1 + 0.0000001 * age - 0.0000001 * weight);
  return maderFormula + sprintContribution;
}
```

March 25th 2023 - updated the Vlamax calculation to include a 20 sec all out sprint test so it is more accuarte. Also added BMI calcualation.
